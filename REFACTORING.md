...
## Этап 1 — Слой транспорта (P0, ядро) ✅ (выполнено 2026-05-21)
...
## Этап 2 — Сервисный слой (P1) ✅ (выполнено 2026-05-21)

> **Отклонение от плана.** Пункт 2.5 предполагал удаление `handlers/host/video.py`
> и приём результата только через `ResultBus`. На практике видеофайл обязан
> физически пройти через Telegram: только так бот получает свой (bot-side)
> `file_id`. Поэтому `host/video.py` **остаётся** (переписан на JSON-протокол),
> `IsHost`-фильтр тоже остаётся. Через `result_queue` идут **только ошибки**;
> успешная доставка — в `host/video.py`. Пункт 4.3 (удаление `IsHost`) отменён.
...
## Этап 3 — Userbot как worker (P0/P1) ✅ (выполнено 2026-05-21)
...
## Этап 4 — Чистка и качество (P2/P3) ✅ (выполнено 2026-05-21)

> Примечания по факту: 4.1 (`captions`) и 4.6 (логирование) выполнены попутно
> в этапах 2–3. 4.2 — таблица `links` оставлена как durable-backup; warm-load
> не нужен, т.к. `download_service._lookup_cache` уже делает fallback-чтение
> из БД при холодном Redis. 4.3 — `IsHost` НЕ удаляется (нужен); вместо этого
> убраны мёртвые `db.set_status/see_status` и ленив `utils/__init__.py`.
